...
class TestExercises:
    ...
    def test_create_exercise(self, function_course: CourseFixture, exercises_client: ExercisesClient):
        ...
        response = exercises_client.create_exercise_api(create_exercise_request)
        response_data = CreateExerciseResponseSchema.model_validate_json(response.text)

        assert_status_code(response.status_code, HTTPStatus.OK)
        assert_create_exercise_response(create_exercise_request, response_data)
    ...
    def test_delete_exercise(self, exercises_client: ExercisesClient, function_exercise: ExerciseFixture):
        ...
        request = exercises_client.get_exercise_api(function_exercise.response.exercise.id)
        assert_status_code(request.status_code, HTTPStatus.NOT_FOUND)
        assert_exercise_not_found_response(request)
